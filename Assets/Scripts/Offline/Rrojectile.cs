...
public class Rrojectile : MonoBehaviour {
	public float lifeTime = 2.5f;
	public ParticleEmitter[] trailEmitters;
	public AudioSource audioSource;
	public AudioClip explosionSound;
	public GameObject explosionPrefab;
	public GTA_Controller rr;
	...
	void FixedUpdate () {
		...
		if(Physics.Raycast(transform.position, transform.TransformDirection(Vector3.forward), out hit, 2.0f)){
			...
			foreach(WeaponSystemSetup.HIT_DAMAGE_BODY fireDamage in rr.weapons.slots[(int)rr.weapons.currentSlot].DamageBodyParts){
			...
			if(hit.rigidbody){
				foreach(WeaponSystemSetup.EffectsPhysics lHit in rr.weapons.LayersEffectsPhysics){
